...
def evaluate_class_mdl(fitted_model, X_train, X_test, y_train, y_test, plot_roc=True, plot_conf_matrix=False,\
                       pct_matrix=True, predopts={}, show_summary=True, ret_eval_dict=False, save_name=None):
...
def evaluate_reg_mdl(fitted_model, X_train, X_test, y_train, y_test, scaler=None,\
                     ...
                     ret_eval_dict=False, predopts={}, save_name=None):
...
def plot_3dim_decomposition(Z, y_labels, y_names, save_name=None):
    if len(y_names) > 2:
        cmap = 'plasma_r'
    else:
        cmap = 'viridis'
...
def create_decision_plot(X, y, model, feature_index, feature_names, X_highlight,\
    ...
    if list(np.arange(X.shape[1]+con))!=list(filler_feature_values.keys()):
        if add_constant:
            ...
            fillers= {}
        fillers.update({X.columns.get_loc(key)+con:vals for key, vals in filler_feature_values.items()})
    ...
    if sum([True for k in feature_index if k not in filler_feature_keys])==2:
        feature_index = [X.columns.get_loc(k)+con for k in feature_index if k not in filler_feature_keys]
...
def compare_img_pred_viz(img_np, viz_np, y_true, y_pred, probs_s=None, title=None, save_name=None):
...
def compare_confusion_matrices(y_true_1, y_pred_1, y_true_2, y_pred_2, group_1, group_2,\
                               plot=True, compare_fpr=False, save_name=None):
...
def compare_image_predictions(X_mod, X_orig, y_mod, y_orig, y_mod_prob=None, y_orig_prob=None,\
                              ...
                              max_width=14, use_misclass=True, save_name=None):
    if calc_difference:
...
def compare_df_plots(df1, df2, title1=None, title2=None, y_label=None, x_label=None,\
                     y_formatter=None, x_formatter=None, plot_args={}, save_name=None):
